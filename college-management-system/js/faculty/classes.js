 ...
 */
import { COURSE_TYPES, BRANCHES, SEMESTER_STRUCTURE, DEMO_CONTENT, ROUTES, resolvePath } from '../config.js';
...
import { getClasses, createClass, deleteClass } from '../services/classService.js';
...
async function load() {
  allClasses = await getClasses({ facultyId: FACULTY_ID });
  render();
}
...
function render() {
  ...
  const filtered = allClasses.filter((c) => {
    const hay = `${c.courseName || ''} ${c.course} ${c.branch}`.toLowerCase();
    const matchQ = !q || hay.includes(q);
    const matchC = !fc || (c.program || c.course) === fc;
  ...
  $$('[data-del]', host).forEach((btn) =>
    btn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); onDelete(btn.dataset.del); })
  );
}
  ...
  return `
    <div class="classroom-card" role="link" tabindex="0" data-href="${href}">
      <div class="card-header" style="background:${bannerGradient(subject)}">
        ...
        <p class="class-subtitle">${esc(c.program || c.course)} • ${esc(c.branch)} • Sem ${c.semester}</p>
      </div>
      <div class="card-body">
        <div class="student-count">${SVG_USERS}<span>${students} Student${students === 1 ? '' : 's'}</span></div>
      </div>
      <div class="card-footer">
        <button class="btn-delete" data-del="${c.id}" aria-label="Delete ${esc(subject)}" title="Delete class">${SVG_TRASH}</button>
...
async function onDelete(id) {
  ...
  const ok = await confirmDialog({
    title: 'Delete class?',
    message: `This will remove ${c.courseName || c.course} · ${c.branch} · Semester ${c.semester}. This cannot be undone.`,
    confirmLabel: 'Delete',
  });
  if (!ok) return;
...
function openWizard() {
  const state = { step: 1, course: '', branch: '', semester: null, courseName: '', description: '' };
  const LAST = 4;
    ...
    body: `
      ...
      <div class="wizard-panel" data-panel="4">
        <div class="form-group">
          <label class="form-label" for="wzCourseName">Course / subject name <span class="req">*</span></label>
          <input class="input" id="wzCourseName" placeholder="e.g. Compiler Design" />
          <div class="field-error" id="wzCourseNameErr"></div>
        </div>
        <div class="form-group">
          <label class="form-label" for="wzDescription">Description</label>
          <textarea class="input" id="wzDescription" placeholder="Short description of the course (optional)"></textarea>
        </div>
  ...
  function renderSummary() {
    const parts = [];
    if (state.courseName) parts.push(['Course', state.courseName]);
    if (state.course) parts.push(['Program', state.course]);
  ...
  $$('#courseChips .chip', el).forEach((chip) =>
    chip.addEventListener('click', () => {
      ...
      state.semester = null; // reset dependent selection
      $$('#courseChips .chip', el).forEach((c) => c.classList.toggle('selected', c === chip));
      renderSummary();
    })
  );
  // Branch chip selection
  ...
  async function goNext() {
    ...
    if (state.step === 3) {
      ...
      // focus the course name field for a smooth flow
      setTimeout(() => $('#wzCourseName', el)?.focus(), 60);
      return;
    }
    // step 4 -> create
    state.courseName = ($('#wzCourseName', el).value || '').trim();
    state.description = ($('#wzDescription', el).value || '').trim();
    const errEl = $('#wzCourseNameErr', el);
    if (!state.courseName) {
      $('#wzCourseName', el).classList.add('error');
    ...
    toastSuccess(`Class created: ${state.courseName} · ${state.branch} · Semester ${state.semester}`);
    close();
    await load();
  }
